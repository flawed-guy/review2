...
function Map({ userLocation, ambulanceLocation, routeGeometry }) {
  ...
  useEffect(() => {
    ...
    const newPosition = [Number(ambulanceLocation.longitude), Number(ambulanceLocation.latitude)];

    if (!ambulanceMarker.current) {
      const element = document.createElement("div");
      element.textContent = "🚑";
      element.style.fontSize = "32px";
      ambulanceMarker.current = new maplibregl.Marker({ element }).setLngLat(newPosition).addTo(map.current);
      return;
    }
    ...
    const endLng = newPosition[0];
    const endLat = newPosition[1];
    const duration = 1000;
